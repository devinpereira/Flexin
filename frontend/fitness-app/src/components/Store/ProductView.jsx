...
const ProductView = ({ onAddToCart, productData, favorites, onToggleFavorite, onBackToStore, addedToCartItems = new Set(), onProductClick }) => {
  const { productId } = useParams();
  const location = useLocation();
  const navigate = useNavigate();

  // Use productData from props if available, otherwise fall back to location state or URL params
  const initialProduct = productData || location.state?.product || null;
  const initialProductId = productData?.id || productId;

  console.log('ProductView loaded with productId:', initialProductId);
  ...
  const [localAddedToCartItems, setLocalAddedToCartItems] = useState(new Set()); // Local state for standalone mode

  // Use parent addedToCartItems if available (child mode), otherwise use local state (standalone mode)
  const currentAddedToCartItems = onAddToCart ? addedToCartItems : localAddedToCartItems;
  ...
  const handleToggleFavorite = () => {
    if (onToggleFavorite && product) {
      ...
      // Standalone mode - just toggle local state
  ...
  // Handle add to cart (exactly like MainStoreView - self-sufficient)
  const handleAddToCart = async () => {
    ...
    try {
      ...
      // Call the parent's onAddToCart function if available (child mode - parent handles visual feedback)
      if (onAddToCart) {
        onAddToCart(formattedProduct, quantity);
      } else {
        // Standalone mode - handle visual feedback locally
        setLocalAddedToCartItems(prev => new Set([...prev, formattedProduct.id]));

        // Remove highlight after 2 seconds
        setTimeout(() => {
          setLocalAddedToCartItems(prev => {
        ...
        // Update localStorage directly like MainStoreView pattern
        ...
        console.log('Updated localStorage cart in standalone mode:', cartItems);
      }

      // Show notification (ProductView specific)
      ...
      if (onAddToCart) {
        ...
        // Standalone mode - handle visual feedback and localStorage update even on error
        setLocalAddedToCartItems(prev => new Set([...prev, formattedProduct.id]));
        setTimeout(() => {
          setLocalAddedToCartItems(prev => {
            const newSet = new Set(prev);
        ...
        // Fallback localStorage update even on error for standalone mode
        const savedCartItems = localStorage.getItem('cartItems');
        let cartItems = [];
        if (savedCartItems) {
          cartItems = JSON.parse(savedCartItems);
  ...
  const handleProductClick = (relatedProduct) => {
    ...
    if (onProductClick) {
      console.log('Using parent onProductClick callback for related product navigation');
      onProductClick(relatedProduct);
    } else {
      console.log('No parent callback, using direct navigation for related product');
  ...
  if (!product) {
    return (
      <div className="w-full text-white p-8 text-center">
        <p>Product not found.</p>
        <button
          onClick={onBackToStore}
          className="mt-4 bg-[#f67a45] hover:bg-[#e56d3d] text-white py-2 px-6 rounded-lg transition-colors"
        >
          Back to Store
        </button>
      </div>
        ...
        <div className="lg:w-1/2 min-w-0">
          <div className="bg-[#121225] border border-[#f67a45]/30 rounded-lg p-6">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 mb-6">
              ...
              <button
                className={`flex-1 py-2 px-6 rounded-lg transition-all duration-300 flex items-center justify-center gap-2 ${currentAddedToCartItems.has(formatProduct(product)?.id)
              ...
              >
                {currentAddedToCartItems.has(formatProduct(product)?.id) ? (
                  <>
