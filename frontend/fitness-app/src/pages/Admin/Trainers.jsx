...
const Trainers = () => {
  ...
  // Fetch trainers from backend on mount
  useEffect(() => {
    async function fetchTrainers() {
      try {
        const res = await axiosInstance.get("/api/v1/trainers");
        // If your API returns { trainers: [...] }
        setAllTrainers(res.data.trainers || res.data);
        console.log("Trainers loaded:", res.data.trainers || res.data);
      } catch (err) {
  ...
  const handleDeleteTrainer = (trainer) => {
    setConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          // API call to delete trainer
          await axiosInstance.delete(`/api/v1/trainers/${trainer._id}`);
          // Remove from local state
          setAllTrainers((prev) => prev.filter((t) => t._id !== trainer._id));
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          ...
          await axiosInstance.put(`/api/v1/trainers/${trainer._id}`, {
            status: newStatus,
          });
          // Update local state
          setAllTrainers((prev) =>
          ...
          showSuccess(
            `Trainer ${trainer.name}'s status has been changed to ${newStatus}.`
          );
        } catch (err) {
    ...
    <AdminLayout pageTitle="Trainer Management">
      ...
      <div className="border-b border-gray-700 mb-6">
        <div className="flex flex-wrap -mb-px">
          ...
          >
            Active
            <span className="ml-2 bg-green-900/30 text-green-400 px-2 py-0.5 rounded-full text-xs">
              {counts.active}
            </span>
          </button>
          <button
          ...
          >
            Pending
            <span className="ml-2 bg-yellow-900/30 text-yellow-400 px-2 py-0.5 rounded-full text-xs">
              {counts.pending}
            </span>
          </button>
        </div>
                    ...
                    <td className="py-3 px-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex justify-end gap-2">
                        ...
                        {trainer.status === "active" ? (
                          <button
                            onClick={() =>
                              handleStatusChange(trainer, "inactive")
                            }
                          ...
                          <button
                            onClick={() =>
                              handleStatusChange(trainer, "active")
                            }
      ...
      <div className="mt-6 bg-blue-900/20 border border-blue-500/30 rounded-lg p-4">
        ...
        <ul className="text-white/70 text-sm space-y-1 list-disc list-inside">
          <li>
            Review pending trainer applications regularly from the 'Approve
            Trainers' page
          </li>
          ...
          <li>
            Ensure all trainers have complete profiles with proper
            certifications
          </li>
        </ul>
      </div>
