...
const AddSchedule = () => {
  ...
  // Load schedule data from localStorage on component mount
  useEffect(() => {
    // Load schedule data from localStorage if it exists
    const savedScheduleData = localStorage.getItem('currentScheduleInProgress');
    if (savedScheduleData) {
      try {
        ...
        if (parsed.exercises) {
          setExercises(() => {
            // Create a new object with all weekdays initialized
            ...
            // Add exercises from localStorage
            Object.keys(parsed.exercises).forEach(day => {
              if (Array.isArray(parsed.exercises[day])) {
                updatedExercises[day] = parsed.exercises[day];
              }
            });
          ...
          // Select the first day that has exercises
          const firstDayWithExercises = Object.keys(parsed.exercises).find(
            day => parsed.exercises[day]?.length > 0
          );
        ...
        displayAlert('Error loading saved progress', 'error');
      }
    }
  }, []);
  ...
  // Update schedule name and sync with localStorage
  const updateScheduleName = (newName) => {
    setScheduleName(newName);
    
    // Update localStorage
    try {
      const savedData = localStorage.getItem('currentScheduleInProgress');
      if (savedData) {
      ...
      console.error('Error updating schedule name in localStorage:', error);
    }
  };
  ...
  const handleUpdateExercise = (day, exerciseId, updatedData) => {
    setExercises(prev => {
      ...
      try {
        ...
        console.error('Error updating exercise in localStorage:', error);
      }
  ...
  const handleDeleteExercise = (day, exerciseId) => {
    setExercises(prev => {
      const updatedDay = prev[day].filter(exercise => exercise.id !== exerciseId);
      ...
      try {
        ...
        console.error('Error deleting exercise in localStorage:', error);
      }
  ...
  const handleSaveSchedule = () => {
    ...
    try {
      const existingSchedules = JSON.parse(localStorage.getItem('customSchedules')) || [];
      localStorage.setItem('customSchedules', JSON.stringify([...existingSchedules, schedule]));
      
      // Clear the in-progress schedule data
      localStorage.removeItem('currentScheduleInProgress');
      
      displayAlert('Schedule saved successfully!', 'success');
                ...
                {editingName ? (
                  <div className="flex items-center gap-2">
                    <input 
                      type="text"
                      value={scheduleName}
                      onChange={(e) => updateScheduleName(e.target.value)}
              ...
              <div className="bg-[#121225] border border-[#f67a45]/30 rounded-lg p-6 sticky top-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="pt-4 border-t border-white/10">
                    ...
                    {Object.values(exercises).every(day => day.length === 0) && (
