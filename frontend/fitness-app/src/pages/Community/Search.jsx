    ...
    try {
      ...
      setSearchResults((prevResults) =>
        prevResults.map((user) =>
          user.id === userId ? { ...user, isFollowing: isFollowing === "accepted" ? false : "accepted" } : user
        )
      );
    } catch (err) {
            ...
            >
              {searchResults.map((user) => (
                ...
                >
                  ...
                  <button
                    className={`px-4 py-2 rounded-full ${user.isFollowing === "accepted"
                        ? "bg-gray-700 text-white"
                        : "bg-[#f67a45] text-white hover:bg-[#e56d3d]"
                      }`}
                    onClick={(e) => handleFollowUser(user.id, user.isFollowing, e)}
                  >
